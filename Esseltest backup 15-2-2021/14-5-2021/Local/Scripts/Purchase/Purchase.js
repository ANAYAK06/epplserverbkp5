    ...
    $.ajax({
        ...
        success: function (Data) {
            if (Data.saveStatus === 'Submited') {
                ...
                $("#divNewSppoInfoMsg").addClass("alert-danger");
                $("#divNewSppoInfoMsg").removeClass("hidden alert-success");
                $("#btnSubmitNewSPPO").prop("disabled", false);
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
...
function ApproveSPPOAmend(AmendId, SPPONo, VCode, CCCode, AmendDate, DCACode, AmendAmount, SubstractAmount) {
    ...
    $("#AmenditemDescTable tbody tr").each(function () {
        ...
        var check = currentRow.find("td").eq(0).find('input[type="checkbox"]').is(':checked');
        if (check == false) {
            checkccount = checkccount + 1;
        }
    });
    var taccheckcount = 0, tcacount = 0, termscount = 0;
    termscount = $("#ApprTACTable tbody tr").length;
    $("#ApprTACTable tbody tr").each(function () {
        var currentRow = $(this);
        var check = currentRow.find("td").eq(0).find('input[type="checkbox"]').is(':checked');
        var tca = currentRow.find("td").eq(2).find("textarea").val();
        if (check === true) { taccheckcount = taccheckcount + 1; }
        if (tca === "") { tcacount = tcacount + 1; }
    });
    if (checkccount > 0) {
    ...
    if (taccheckcount !== termscount) {
        errorMsg += "<p style='margin-top:-5px!important;'>Please verify Trems And Conditions </p>";
        isValid = false;
    }
    else if (tcacount > 0) {
        errorMsg += "<p style='margin-top:-5px!important;'>Enter Trems And Conditions</p>";
        isValid = false;
    }
    if (!isValid) {
        var finalerror1 = "<div style='align:center'><p>Please enter all required fields!</p>";
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (advance == 0 || advance == '0') {
                    $("#txtNSpAdvance").val(0);
                    $("#txtNSpAdvance").prop("disabled", true);
                    //$("#txtNSpAdvance").addClass("tooltip");
                    $("#txtNSpAdvance").css("background-color", "transparent");
                    //$("#txtNSpAdvance").tooltip({
                    //    items: "#txtNSpAdvance",
                    //    content: "Age should be between 1 and 100."
                    //});
                    $("#txtNSpAdvance").attr('title', 'Advance Have Not Taken For This PO');
                    // $("#txtNSpAdvance").tooltip("open");
                }
                else {
                    $("#txtNSpAdvance").val(parseFloat(advance));
                    $("#txtOriginalAdvance").val(parseFloat(advance));
                    //$("#txtNSpAdvance").tooltip("disable");
                }
            },
            failure: function (response) {
...
function ApproveSPPOInvoice(InvoiceData) {
    ...
    if (!isValid) {
        ...
        var ApproveInvoice = {
            ...
            TaxApplicable: InvoiceData.TaxApplicable
        };
        //debugger;
        addFailMsg = "Error Occurred";
        var finalmsg;
...
function ShowSupplierReport() {
    ...
    if (searchindex === 0) {
        ...
        if (search === "CC") {
            var options = $('#ddlSuppoCC > option:selected');
            if (options.length == 0) {
                errorMsg += "<p style='margin-top:-5px!important;'>Select Cost Center</p>";
                isValid = false;
            }
    ...
    if (!isValid) {
        ...
        var SelectedCC = $('#ddlSuppoCC option:selected').toArray().map(item => item.value).join() + ',';
        $.ajax({
            type: 'GET',
            dataType: 'html',
            url: '/Reports/SupplierPOReportGrid',
            data: { CCCode: SelectedCC, FromDate: from, ToDate: To, Searchby: search },
            success: function (Data) {
                var count1 = Object.keys(Data).length;
                var msg = "";
                if (count1 > 0) {
