...
function ddlClientActionschange(clientid, clientcode, clientname, clientgroup, clientgroupid, action) {
    ...
    if (selectedaction == "UpdateClient") {
        ...
        ClearAddSubClient();
        $("#txtnaturegroupid").val(clientgroupid);
        $("#txtnaturegroup").val(clientgroup);
...
function SubmitSubClientData() {
    ...
    var naturegroupid = $("#txtnaturegroupid").val();
    ...
    $.ajax({
        ...
        success: function (Data) {
            if (Data.saveStatus == true) {
                ...
                $("#btnSubmitClientInvCreation").prop("disabled", false);
...
function ClientRec_BindNewRowtoGrid() {
    ...
    $("#ClientRecDCAGrid tbody tr").each(function () {
        ...
        if (SubDCA === 0) {
            ...
            var SubDCAcode = currentRow.find("td").eq(4).find("select option:selected").val();
            selecteddcalist.push(SubDCAcode);
        }
    ...
    var duplicatelist = selecteddcalist.filter(i => selecteddcalist.filter(ii => ii === i).length > 1);
    if (duplicatelist.length > 0) {
        errorMsg += "<p style='margin-top:-5px!important;'>Duplicate Sub AccountHeads for Deduction </p>";
        isValid = false;
    }
...
function SubmitClientRecievableData() {
    ...
    if (!isValid) {
        ...
        $("#ClientRecDCAGrid tbody tr").each(function () {
            ...
            var dedsdca = currentRow.find("td").eq(4).find("select option:selected").val();
            if (dedsdca !== 0) {
                //selecteddcalist.push(dedcc + '-' + deddca);
                // alert(currentRow.find("td").eq(2).find("select option:selected").val());
                selecteddcalist.push(dedsdca);
            }
        });
        var duplicatelist = selecteddcalist.filter(i => selecteddcalist.filter(ii => ii === i).length > 1);
        ...
        if (duplicatelist.length > 0) {
            var finalerror2 = "<div style='align:center'><p>Duplicate Sub AccountHeads for Deduction</p>";
            $("#divClientRecInfoMsg").text("");
    ...
    $.ajax({
        ...
        success: function (Data) {
            if (Data.saveStatus === "Invoice Submited") {
                ...
                $("#btnClientRecSubmit").prop("disabled", false);
    ...
    $.ajax({
        ...
        success: function (Data) {
            ...
            if (Data.saveStatus == true) {
                ...
                $("#divamendpoInfoMsg").addClass("alert-danger");
                $("#divamendpoInfoMsg").removeClass("hidden alert-success");
                $("#btnClientamendPOSubmit").prop('disabled', false);
            }
    ...
    $.ajax({
        ...
        success: function (Data) {
            if (Data.saveStatus == 'Updated') {
                ...
                $("#divAddSubDCAInfoMsg").addClass("alert-danger");
                $("#divAddSubDCAInfoMsg").removeClass("hidden alert-success");
                $("#editsubdca").prop("disabled", false);
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            $("#divAddSubDCAInfoMsg").text(addFailMsg);
    ...
    $.ajax({
        ...
        success: function (Data) {
            if (Data.saveStatus == "Successfull") {
                ...
                $("#btnSubmitjvCreation").prop("disabled", false);
